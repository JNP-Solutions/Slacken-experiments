...
df_result3=pd.read_csv("/Users/n-dawg/IdeaProjects/Slacken-SBI/scripts/benchmark_data/All_paper_metrics.tsv",sep='\t')
val='read_fn_frac'

for ds in ['strain','marine','plant_associated','Assorted_Genomes_mbarc_225','Assorted_Genomes_225','Assorted_Genomes_Perfect_225']:
    for rank in ['Species']:
        df = df_result3[(df_result3['group']==ds) & (df_result3['c']==0.15) & (df_result3['rank']==rank)].reset_index()
        df = df[['library','sample',val]]
        df.rename(columns={'library':'classifier',val:'ratio'},inplace=True)
        ...
        mean_diff.to_csv(f'/Users/n-dawg/IdeaProjects/Slacken-SBI/scripts/paper_figures/FigureData/'
                         f'pairwise_difference_metrics_fn/pairwise_meanOfDifferences_{ds}_{rank}_{val}.csv',index=False)